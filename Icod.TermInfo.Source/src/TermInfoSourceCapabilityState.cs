...
internal sealed class TermInfoSourceCapabilityState {
	...
	internal TermInfoSourceCapabilityState Inherit(
		...
		foreach ( BooleanCapability capability in source._booleanCapabilities ) {
			if (
				!_booleanCapabilities.Contains( capability )
		...
		foreach (
			...
			if (
				!_numericCapabilities.ContainsKey( pair.Key )
		...
		foreach (
			...
			if (
				!_stringCapabilities.ContainsKey( pair.Key )
		...
		foreach (
			...
			if (
				!_extendedCapabilities.ContainsKey( pair.Key )
		...
		foreach (
			...
			if (
				!_numericCapabilities.ContainsKey( capability )
		...
		foreach (
			...
			if (
				!_stringCapabilities.ContainsKey( capability )
		...
		foreach ( string name in source._canceledExtendedCapabilities ) {
			if (
				!_extendedCapabilities.ContainsKey( name )
	...
	private void ApplyLocalField( TermInfoSourceField field ) {
	...
	private void ApplyStandardField( TermInfoSourceField field ) {
		ArgumentNullException.ThrowIfNull( field );
		...
		switch ( field.Kind ) {
			...
			case TermInfoSourceFieldKind.NumericCapability:
				if (
					field.StandardNumericCapability is NumericCapability numeric
			...
			case TermInfoSourceFieldKind.StringCapability:
				if (
					field.StandardStringCapability is StringCapability text
	...
	private void ApplyExtendedField( TermInfoSourceField field ) {
		ArgumentNullException.ThrowIfNull( field );
	...
	private void SetBoolean( BooleanCapability capability ) {
	...
	private void CancelBoolean( BooleanCapability capability ) {
		_booleanCapabilities.Remove( capability );
		_canceledBooleanCapabilities.Add( capability );
	}

	private void CancelNumber( NumericCapability capability ) {
		_numericCapabilities.Remove( capability );
		_canceledNumericCapabilities.Add( capability );
	}

	private void CancelString( StringCapability capability ) {
		_stringCapabilities.Remove( capability );
		_canceledStringCapabilities.Add( capability );
	}

	private void CancelExtended( string name ) {
		ArgumentException.ThrowIfNullOrWhiteSpace( name );
