...
internal sealed class TermInfoSourceScanner {
	...
	private bool ContainsWhitespace(
		...
		for ( int offset = start; offset < end; offset++ ) {
			if ( char.IsWhiteSpace( _source[offset] ) ) {
				return true;
			}
		}
	...
	private int TrimTrailingInterFieldWhitespace(
		...
		while (
			( end > start )
			&& IsInterFieldWhitespace( _source[end - 1] )
		) {
			end--;
		}
	...
	private bool IsOnlyWhitespace(
		...
		for ( int offset = start; offset < end; offset++ ) {
			if ( !IsInterFieldWhitespace( _source[offset] ) ) {
				return false;
			}
		}
	...
	private static bool IsInterFieldWhitespace( char value ) {
		return ( value == ' ' )
			|| ( value == '\t' )
			|| ( value == '\r' )
			|| ( value == '\n' )
