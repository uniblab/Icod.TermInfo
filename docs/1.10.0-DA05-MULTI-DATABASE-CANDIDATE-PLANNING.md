...
**Status:** DA05 frozen; Debug and cross-host PR Staging validation complete  
...
Audited-head PR Staging run `33714831878` then completed successfully across
Windows, Linux, and macOS. The Linux coordination job created and exactly verified
the Staging package set and all supported tool archives. Installable
`Icod.TermInfo.Tools` smoke tests passed on Windows, Linux, and macOS, and archive
smoke tests passed for `win-x64`, `win-arm64`, `linux-x64`, `linux-arm64`,
