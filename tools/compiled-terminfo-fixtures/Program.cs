/*
	Icod.TermInfo.FixtureGenerator
	Generates compiled terminfo fixtures for repository validation.
...
internal static class Program {
	...
	public static int Main( string[] args ) {
		...
		try {
			...
			if ( checkOnly ) {
				snapshot = CaptureCorpus( fixtureRoot );
			}
			...
			if ( checkOnly ) {
				VerifyCorpusMatches( fixtureRoot, snapshot! );
			...
			if ( checkOnly
				...
				RestoreCorpus( fixtureRoot, snapshot );
			}
		}
	}
	...
	private static void CompileSources( string root ) {
		...
		try {
			...
			string[] sources =
				Directory.EnumerateFiles( sourceDirectory, "*.ti" )
			...
			foreach ( string source in sources ) {
				...
				string terminalName = Path.GetFileNameWithoutExtension( source );
				string[] matches =
		...
		string edgePath = Path.Combine( compiled, "t29-legacy-edge.bin" );
	...
	private static void CreateAdversarialSeeds( string root ) {
		...
		byte[] minimal = File.ReadAllBytes(
			Path.Combine( compiled, "t29-legacy-minimal.bin" )
		);
		byte[] extended = File.ReadAllBytes(
		...
		int collisionOffset = LastIndexOf(
			collision,
	...
	private static int ConventionalEnd( byte[] data ) {
		...
		if ( magic != LegacyMagic && magic != ExtendedNumberMagic ) {
			throw new InvalidDataException(
	...
	private static Dictionary<string, byte[]> CaptureCorpus( string root ) {
		ArgumentException.ThrowIfNullOrWhiteSpace( root );
		...
		foreach ( string folder in new[] { "compiled", "malformed" } ) {
			...
			foreach (
				string path
				in Directory.EnumerateFiles( directory, "*.bin" )
					.OrderBy( path => path, StringComparer.Ordinal )
			) {
				string relative =
	...
	private static void VerifyCorpusMatches(
		...
		Dictionary<string, byte[]> actual = CaptureCorpus( root );
		...
		foreach ( string name in expectedNames ) {
			if ( !expected[name].AsSpan().SequenceEqual( actual[name] ) ) {
				throw new InvalidDataException(
	...
	private static void RestoreCorpus(
		...
		foreach (
			...
			in snapshot.OrderBy( pair => pair.Key, StringComparer.Ordinal )
		) {
