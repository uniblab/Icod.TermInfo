...
printf '%s\n' "== HDB07C macOS: produce and verify native Latin-1 store =="
run_hashed_tic -x -o "$latin1_hashed_base" "$latin1_fixture"
test -f "$latin1_hashed_db"
DYLD_LIBRARY_PATH="$db_prefix/lib${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}" \
    "$db_prefix/bin/db_dump" -k -f "$latin1_dump" "$latin1_hashed_db"
dotnet run \
    --project "$native_verifier_project" \
    -c Release \
    -- \
