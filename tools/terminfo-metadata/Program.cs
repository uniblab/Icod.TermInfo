/*
	Icod.TermInfo.MetadataGenerator
	Generates terminfo capability metadata for repository sources.
...
internal static class Program {
	...
	public static int Main( string[] args ) {
		...
		try {
			...
			IReadOnlyList<Capability> capabilities = ParseSource( sourcePath );
	...
	private static void ValidateEnumNames(
		...
		string source = NormalizeLineEndings( File.ReadAllText( enumPath ) );

		HashSet<string> actualNames =
			EnumMemberPattern.Matches( source )
				.Cast<Match>()
		...
		string[] missing =
			expectedNames.Except( actualNames, StringComparer.Ordinal )
				.OrderBy( value => value, StringComparer.Ordinal )
				.ToArray();
		string[] extra =
			actualNames.Except( expectedNames, StringComparer.Ordinal )
				.OrderBy( value => value, StringComparer.Ordinal )
				.ToArray();
	...
	private static string Generate(
		...
		AppendLine( builder, "/*" );
		AppendLine( builder, "\tIcod.TermInfo" );
		AppendLine(
			builder,
	...
	private static int CheckGeneratedFile(
		...
		string current = NormalizeLineEndings( File.ReadAllText( outputPath ) );
