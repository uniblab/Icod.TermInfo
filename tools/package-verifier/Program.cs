...
internal static class Program {
	...
	private static string VerifyPrimaryPackage(
		...
		foreach ( string targetFramework in TargetFrameworks ) {
			required.Add( $"lib/{targetFramework}/Icod.TermInfo.dll" );
			required.Add( $"lib/{targetFramework}/Icod.TermInfo.xml" );
		}
		...
		string[] expectedDlls =
			TargetFrameworks
				.Select(
					targetFramework => $"lib/{targetFramework}/Icod.TermInfo.dll"
				)
				.OrderBy( name => name, StringComparer.Ordinal )
				.ToArray();
		Require(
		...
		Require(
			string.Equals(
				GetMetadataText( metadata!, "requireLicenseAcceptance" ),
				"true",
				StringComparison.OrdinalIgnoreCase
			),
		...
		Require(
			!string.IsNullOrWhiteSpace( GetMetadataText( metadata!, "description" ) ),
			"Package description is missing."
		);
		Require(
			!string.IsNullOrWhiteSpace( GetMetadataText( metadata!, "tags" ) ),
	...
	private static void VerifyDocumentation(
		...
		string documentationPath = $"lib/{targetFramework}/Icod.TermInfo.xml";
		ZipArchiveEntry? entry = package.GetEntry( documentationPath );
		Require(
			entry is not null,
			$"Primary package is missing {documentationPath}."
		);
		Require( entry!.Length > 0, $"{documentationPath} is empty." );
		...
		int memberCount =
			documentation.Descendants()
				.Count( element => element.Name.LocalName == "member" );
	...
	private static void VerifyAssemblyIdentity(
		...
		string assemblyPath = $"lib/{targetFramework}/Icod.TermInfo.dll";
	...
	private static void VerifySymbolPackage(
		...
		foreach ( string targetFramework in TargetFrameworks ) {
			string pdbPath = $"lib/{targetFramework}/Icod.TermInfo.pdb";
