/*
	Icod.TermInfo.Compiler.PackageVerifier
...
namespace Icod.TermInfo.Compiler.PackageVerifier;
...
internal static class Program {
	...
	public static int Main(
		...
		if ( args.Length > 1 ) {
			Console.Error.WriteLine(
				"Usage: dotnet run --project tools/compiler-package-verifier/Icod.TermInfo.Compiler.PackageVerifier.csproj -- [artifact-directory]"
			);
			return 2;
		}
	...
	private static string VerifyPackage(
		...
		Require(
			missing.Length == 0,
			"Compiler package is missing required entries: "
				+ string.Join( ", ", missing )
		);
		...
		Require(
			...
			"Compiler package contains unexpected DLL payloads: "
				+ string.Join( ", ", dlls )
		);
		Require(
			!names.Any( name => name.StartsWith( "runtimes/", StringComparison.Ordinal ) ),
			"Compiler package unexpectedly contains a runtimes/ payload."
		);
		...
		XElement[] groups =
			metadata
				...
				.ToArray();
		Require( groups.Length == TargetFrameworks.Length, "Compiler dependency groups do not match the supported target frameworks." );
		foreach ( string targetFramework in TargetFrameworks ) {
			XElement? group =
	...
	private static void VerifyAssemblyIdentity(
		...
		Require( entry is not null, $"Compiler package is missing {path}." );
		string temporaryPath =
			Path.Combine(
				Path.GetTempPath(),
				$"Icod.TermInfo.Compiler-package-verifier-{Guid.NewGuid():N}.dll"
			);
		try {
	...
	private static void VerifyDocumentation(
		...
		Require( documentation.Descendants().Any( element => element.Name.LocalName == "member" ), $"{path} contains no documented members." );
	}
	...
	private static ZipArchiveEntry AssertSingleNuspec(
		...
		ArgumentNullException.ThrowIfNull( package );
		ZipArchiveEntry[] entries = package.Entries.Where( entry => entry.FullName.EndsWith( ".nuspec", StringComparison.OrdinalIgnoreCase ) ).ToArray();
