...
string externalName = "package-smoke-external";
string externalAlias = "package-smoke-external-alias";
byte[] compiledEntry =
...
TerminalDescription parsed =
	CompiledTermInfoParser.Parse( compiledEntry );
Require(
...
string? originalTermInfo = Environment.GetEnvironmentVariable( "TERMINFO" );
string? originalTermInfoDirs = Environment.GetEnvironmentVariable( "TERMINFO_DIRS" );
...
try {
	...
	Environment.SetEnvironmentVariable( "TERMINFO", acquisitionRoot );
	...
	Environment.SetEnvironmentVariable( "TERMINFO_DIRS", originalTermInfoDirs );
