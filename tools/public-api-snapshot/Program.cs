/*
	...
	Generates and compares public API snapshots for Icod.TermInfo.
...
internal static class Program {
	...
	public static int Main( string[] args ) {
		...
		if ( args.Length == 3 ) {
			...
			manifest =
				CreateManifestFromAssemblyPath( assemblyPath );
		} else {
			manifest =
				CreateManifest(
					typeof( TerminalDescription ).Assembly
		...
		if (
			...
			Console.WriteLine( $"SHA-256 {ComputeSha256( manifest )}" );
			return 0;
		}
		...
		Console.WriteLine( $"Public API matches {path}" );
		Console.WriteLine( $"SHA-256 {ComputeSha256( manifest )}" );
		return 0;
	}
	...
	private static int CompareAssemblies(
		...
		string first =
			CreateManifestFromAssemblyPath( firstPath );
		string second =
			CreateManifestFromAssemblyPath( secondPath );
		...
		Console.WriteLine( $"SHA-256 {ComputeSha256( first )}" );
		return 0;
	}
	...
	private static string CreateManifestFromAssemblyPath(
		...
		try {
			...
			return CreateManifest( assembly );
		} finally {
			context.Unload();
		}
	}
	...
	private static string CreateManifest(
		...
		builder.AppendLine( $"# {simpleName} public API baseline" );
		builder.AppendLine(
			"# Format: Icod.TermInfo.PublicApiSnapshot/v1"
		);
		builder.AppendLine( $"# AssemblyVersion: {assemblyVersion}" );
		builder.AppendLine();
	...
	private static void AppendType(
		...
		if (
			...
			builder.Append( "  BASE " );
			builder.AppendLine( FormatType( type.BaseType ) );
		}
		...
		if ( type.IsEnum ) {
			...
			foreach (
				...
				builder.AppendLine( FormatField( field ) );
			}
		}
		...
		foreach (
			...
			in type
				...
				.Where(
					constructor => IsApiVisible( constructor )
				)
				.OrderBy(
					FormatConstructor,
					StringComparer.Ordinal
		...
		foreach (
			...
			builder.AppendLine( FormatEvent( eventInfo ) );
		}
	...
	private static string FormatTypeModifiers(
		...
		string attributes =
			FormatRelevantAttributes( type.CustomAttributes );
	...
	private static string FormatField(
		...
		string attributes =
			FormatRelevantAttributes( field.CustomAttributes );
		if ( !string.IsNullOrEmpty( attributes ) ) {
			result +=
				$" attrs={attributes}";
		}
	...
	private static string FormatProperty(
		...
		if ( getter is not null && IsApiVisible( getter ) ) {
			accessors.Add(
				$"{FormatAccessibility( getter )} get;"
			);
		}

		if ( setter is not null && IsApiVisible( setter ) ) {
			string setterText =
		...
		string attributes =
			FormatRelevantAttributes( property.CustomAttributes );
	...
	private static string FormatEvent(
		...
		if ( addMethod is not null && IsApiVisible( addMethod ) ) {
			accessors.Add(
				$"{FormatAccessibility( addMethod )} add;"
			);
		}

		if ( removeMethod is not null && IsApiVisible( removeMethod ) ) {
			accessors.Add(
		...
		string attributes =
			FormatRelevantAttributes( eventInfo.CustomAttributes );
	...
	private static string FormatParameter(
		...
		string attributes =
			FormatRelevantAttributes( parameter.CustomAttributes );
		if ( !string.IsNullOrEmpty( attributes ) ) {
			result +=
				$" attrs={attributes}";
		}
		...
		foreach ( Type parameter in genericParameters ) {
			...
			foreach (
				...
				if ( constraint != typeof( ValueType ) ) {
					constraints.Add( FormatType( constraint ) );
				}
			}
	...
	private static string FormatNullability(
		...
		StringBuilder builder = new();
		builder.Append(
			FormatNullabilityState( info.ReadState )
		);
		builder.Append( '/' );
		builder.Append(
			FormatNullabilityState( info.WriteState )
		);
	...
	private static void PrintUsage() {
		...
		Console.Error.WriteLine( "  public-api-snapshot" );
		Console.Error.WriteLine(
