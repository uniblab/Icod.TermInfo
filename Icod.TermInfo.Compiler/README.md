...
`Icod.TermInfo` and `Icod.TermInfo.Source` packages. Version 1.4.0 coordinates
Compiler with the stable `tic` and tool-suite release without changing the
frozen 1.2 Compiler public API. The dependency remains one-way; neither Source
nor Runtime depends on Compiler.
