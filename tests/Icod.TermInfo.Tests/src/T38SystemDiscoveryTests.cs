...
public sealed class T38SystemDiscoveryTests {
	...
	public void EncodedTermInfoPrecedesDirectorySearch() {
		...
		string home = temporary.CreateSubdirectory( "home" );
		string searchRoot = temporary.CreateSubdirectory( "search" );
		string defaultRoot = temporary.CreateSubdirectory( "default" );
		...
		byte[] encodedEntry = CreateEntryWithColumns( 80 );
		SystemTerminalDiscoverySnapshot snapshot =
			CreateSnapshot(
				temporary.Root,
				termInfo: "hex:" + Convert.ToHexString( encodedEntry ),
				termInfoDirs: "search",
				homeDirectory: home
			);
		...
		AssertColumns( 80, Load( provider, name ) );
	}
	...
	public void EncodedIdentityMismatchFallsThroughToDirectories() {
		...
		string home = temporary.CreateSubdirectory( "home" );
		byte[] encodedEntry =
			ReadFixture( "compiled/t29-legacy-minimal.bin" );
		...
		TerminalDescription terminal = Load( provider, name );
	...
	public void ExplicitTermInfoDirectoryPrecedesUserAndSearchList() {
		using TemporaryDirectory temporary = new();
		string name = "t29-legacy-minimal";
		string explicitRoot = temporary.CreateSubdirectory( "explicit" );
		string home = temporary.CreateSubdirectory( "home" );
		string searchRoot = temporary.CreateSubdirectory( "search" );
		...
		SystemTerminalDescriptionProvider provider =
			CreateProvider( snapshot, Array.Empty<string>() );

		AssertColumns( 81, Load( provider, name ) );
	}
	...
	public void MissingExplicitTermInfoDirectoryFallsThroughToUserDatabase() {
		...
		AssertColumns( 82, Load( provider, name ) );
	}
	...
	public void TermInfoDirsPreservesDeclaredOrder() {
		using TemporaryDirectory temporary = new();
		string name = "t29-legacy-minimal";
		string first = temporary.CreateSubdirectory( "first" );
		string second = temporary.CreateSubdirectory( "second" );

		WriteLiteralCandidate( first, name, CreateEntryWithColumns( 83 ) );
		WriteLiteralCandidate( second, name, CreateEntryWithColumns( 84 ) );
		...
		AssertColumns( 83, Load( provider, name ) );
	}
	...
	public void EmptyTermInfoDirsComponentExpandsDefaultsInPlace() {
		...
		string name = "t29-legacy-minimal";
		string defaultRoot = temporary.CreateSubdirectory( "default" );
		string after = temporary.CreateSubdirectory( "after" );
		...
		AssertColumns( 84, Load( provider, name ) );
	}
	...
	public void ExplicitTermInfoFileIsNotHiddenAsDirectoryMiss() {
		using TemporaryDirectory temporary = new();
		string name = "t29-legacy-minimal";
		string explicitFile = Path.Combine( temporary.Root, "terminfo.db" );
		string home = temporary.CreateSubdirectory( "home" );
		...
		Assert.Contains( "not a directory tree", exception.Message );
	}
	...
	public void DisablingEnvironmentIgnoresAllEnvironmentSources() {
		...
		WriteLiteralCandidate( explicitRoot, name, CreateEntryWithColumns( 81 ) );
		WriteLiteralCandidate( searchRoot, name, CreateEntryWithColumns( 83 ) );
		WriteLiteralCandidate(
			Path.Combine( home, ".terminfo" ),
			name,
			CreateEntryWithColumns( 82 )
	...
	public void DisablingSystemDatabasesSuppressesFinalAndEmptyComponentDefaults() {
		...
		WriteLiteralCandidate( defaultRoot, name, CreateEntryWithColumns( 84 ) );
	...
	private static byte[] CreateEntryWithColumns( int columns ) {
		byte[] entry = ReadFixture( "compiled/t29-legacy-minimal.bin" );
		SetLegacyColumns( entry, columns );
		return entry;
	}
	...
	private static byte[] CreateRenamedMinimalEntry() {
		...
		Span<byte> names = entry.AsSpan( CompiledHeaderSize, namesSize );

		int firstSeparator = names.IndexOf( (byte)'|' );
		if ( firstSeparator <= 0
	...
	private static void SetLegacyColumns(
		...
		BinaryPrimitives.WriteInt16LittleEndian(
			entry.AsSpan( numericOffset, sizeof( short ) ),
	...
	private static string WriteLiteralCandidate(
		...
		string directory = Path.Combine( root, name[0].ToString() );
