...
public sealed class T37DiscoveryInputTests {
	...
	public void NonEncodedTermInfoValueIsLeftForT38( string value ) {
		Assert.False(
			SystemTerminalDiscoveryInputs.TryLoadEncodedTermInfo(
				value,
				"xterm",
	...
	[InlineData( "b64:A===" )]
	[InlineData( "b64:AA*A" )]
	public void InvalidEncodedSyntaxIsFormatError( string value ) {
	...
	[InlineData( true )]
	[InlineData( false )]
	public void EncodedPayloadIsBoundedBeforeParsing( bool useHex ) {
		byte[] entry =
			ReadFixture(
				"compiled/t29-legacy-minimal.bin"
			);
	...
	public void RelativeRootsUseSnapshottedCurrentDirectory() {
		...
		string root = Assert.Single( roots );
		Assert.Equal(
			Path.GetFullPath(
				"relative",
				currentDirectory
