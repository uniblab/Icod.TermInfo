...
public sealed class TerminalEnvironmentTests {
	...
	public void TerminalNameReturnsExactNonBlankValue(
		...
		using EnvironmentVariableScope scope = new( "TERM", value );
	...
	[InlineData( "" )]
	[InlineData( "   " )]
	public void TerminalNameTreatsMissingOrBlankValueAsUnavailable( string? value ) {
		using EnvironmentVariableScope scope = new( "TERM", value );
	...
	public void InvalidStandardStreamIsRejected() {
		TerminalStandardStream invalid = (TerminalStandardStream)int.MaxValue;
	...
	public void EnvironmentSizeUsesPositiveColumnsAndLines() {
		using EnvironmentVariableScope columns = new( "COLUMNS", "132" );
		using EnvironmentVariableScope lines = new( "LINES", "43" );
	...
	public void InvalidEnvironmentSizeDoesNotProduceFallback(
		...
		using EnvironmentVariableScope columns = new( "COLUMNS", columnsValue );
		using EnvironmentVariableScope lines = new( "LINES", linesValue );
	...
	public void LiveSizeQueriesAreGracefulWithoutInteractiveTty() {
		foreach ( TerminalStandardStream stream in Enum.GetValues<TerminalStandardStream>() ) {
			bool success =
	...
	public void DefaultLiveSizeQueryNeverSubstitutesProfileDefaults() {
		bool success = TerminalEnvironment.TryGetLiveSize( out TerminalSize size );
