...
public sealed class T29BinaryReadinessTests {
	...
	public void ValidFixtureHeadersUseFrozenLittleEndianFormats() {
		...
		foreach ( JsonElement fixture in manifest.RootElement
			...
			int unalignedNumericOffset = 12 + header.NamesSize + header.BooleanCount;
	...
	public void LegacyEdgeFixturePinsAbsentCanceledPaddingAndHighByteSemantics() {
		...
		int bwIndex = StandardCapabilityCatalog.GetMetadata(
			BooleanCapability.AutoLeftMargin
		).BinaryIndex;
		int linesIndex = StandardCapabilityCatalog.GetMetadata(
			NumericCapability.Lines
		).BinaryIndex;
		int belIndex = StandardCapabilityCatalog.GetMetadata(
			StringCapability.Bell
		).BinaryIndex;
		int kbsIndex = StandardCapabilityCatalog.GetMetadata(
			StringCapability.KeyBackspace
		).BinaryIndex;
		int clearIndex = StandardCapabilityCatalog.GetMetadata(
			StringCapability.ClearScreen
		).BinaryIndex;
		int cupIndex = StandardCapabilityCatalog.GetMetadata(
	...
	public void LegacyMinimalFixturePinsOrdinaryAbsentSentinels() {
		...
		int tabWidthIndex = StandardCapabilityCatalog.GetMetadata(
			NumericCapability.InitialTabWidth
		).BinaryIndex;
		int bellIndex = StandardCapabilityCatalog.GetMetadata(
			StringCapability.Bell
		).BinaryIndex;
	...
	public void ExtendedFixturesPinNcursesCountsAndThirtyTwoBitNumbers() {
		...
		int colorsIndex = StandardCapabilityCatalog.GetMetadata(
			NumericCapability.Colors
		).BinaryIndex;
		int pairsIndex = StandardCapabilityCatalog.GetMetadata(
	...
	public void FixtureSemanticManifestsFitTheCompletedPublicModel() {
		...
		foreach ( JsonElement fixture in manifest.RootElement
			...
			string[] aliases = expected.GetProperty( "aliases" )
				.EnumerateArray()
				.Select( item => item.GetString()! )
				.ToArray();
		...
		TerminalDescription edge = BuildExpectedTerminal(
	...
	public void ExtendedStandardNameCollisionHasAFrozenRejectionTarget() {
		byte[] collision = ReadFixture( "malformed/extended-standard-name-collision.bin" );
		byte[] marker = Encoding.ASCII.GetBytes( "cup\0" );

		Assert.True( collision.AsSpan().IndexOf( marker ) >= 0 );

		TerminalDescriptionBuilder builder = new( "t29-standard-name-collision" );
	...
	private static JsonElement FindFixture( string id ) {
		...
		throw new InvalidOperationException(
			$"Fixture '{id}' is not in the manifest."
		);
	}
	...
	private static CompiledHeader ReadHeader( ReadOnlySpan<byte> bytes ) {
		if ( bytes.Length < 12 ) {
			throw new InvalidDataException(
				"The fixture does not contain a six-short header."
			);
		}
	...
	private static ExtendedHeader ReadExtendedHeader(
		...
		if ( bytes.Length < offset + 10 ) {
			throw new InvalidDataException(
				"The fixture has no complete extended header."
			);
		}
	...
	private static string ReadNames(
		...
		if ( names.Length == 0 || names[^1] != 0 ) {
			throw new InvalidDataException(
				"Fixture names section is not NUL-terminated."
			);
		}
	...
	private static string? ReadString(
		...
		if ( terminator < 0 ) {
			throw new InvalidDataException(
				"Fixture string is not NUL-terminated."
			);
		}
