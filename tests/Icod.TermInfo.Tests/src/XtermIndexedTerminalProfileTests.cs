...
public sealed class XtermIndexedTerminalProfileTests {
	...
	public void BuiltInDatabaseLoadsSelectedIndexedProfilesExactly() {
		...
		foreach ( (string name, TerminalDescription profile, int colors, int pairs) in cases ) {
	...
	public void IndexedProfilesReuseModernXtermCommonKeysAndMetadata() {
		...
		foreach ( TerminalDescription variant in variants ) {
			...
			foreach ( NumericCapability capability in Enum.GetValues<NumericCapability>() ) {
				if ( capability is NumericCapability.Colors or NumericCapability.ColorPairs ) {
					continue;
				}
			...
			foreach ( (string name, TermInfoCapabilityValue value) in TerminalProfiles.Xterm.ExtendedCapabilities ) {
				Assert.True(
