...
public sealed class S02LexerTests {
	...
	public void RepresentativeSourceIsClassifiedWithoutValueDecoding() {
		...
		Assert.Equal( "demo", result.Tokens[ 1 ].Text );
		Assert.Equal( "demo-alt", result.Tokens[ 2 ].Text );
		Assert.Equal( "Demonstration terminal", result.Tokens[ 3 ].Text );
		Assert.Equal( "clear=\\E[H\\,literal", result.Tokens[ 6 ].Text );
		Assert.Equal( "demo.ti", result.Tokens[ 6 ].Span.SourceName );
		Assert.Equal( 5, result.Tokens[ 6 ].Span.Line );
		Assert.Equal( 2, result.Tokens[ 6 ].Span.Column );
	}
	...
	public void CommentsMayAppearBetweenEntryFieldsAndEntries() {
		...
		Assert.Equal( "# retained between entries", comment.Text );
	...
	public void NcursesDisabledCapabilityIsRetainedAsLexicalUnit() {
		...
		Assert.Equal( ".ind=\\E[%p1%dS", disabled.Text );
	}
	...
	public void SourceSpanUsesUtf16OffsetsAndOneBasedLineAndColumn() {
		...
		Assert.Equal( "locations.ti", numeric.Span.SourceName );
	}
	...
	public void OrphanedIndentedFieldProducesDiagnostic() {
		TermInfoSourceLexResult result =
			TermInfoSourceLexer.Tokenize( "\tam,\n" );
	...
	public void EmptyHeaderComponentsProduceStableDiagnostics() {
		TermInfoSourceLexResult result =
			TermInfoSourceLexer.Tokenize( "|alias||,\n" );
	...
	public void PublicContractsValidateProgrammerArguments() {
		Assert.Throws<ArgumentNullException>(
			() =>
				TermInfoSourceLexer.Tokenize( (string)null! )
		);
		Assert.Throws<ArgumentNullException>(
			() =>
				TermInfoSourceLexer.Tokenize( (TextReader)null! )
		);
