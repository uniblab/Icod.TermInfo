...
public sealed class S04ParserTests {
	...
	public void RepresentativeDocumentPreservesUnresolvedEntrySemantics() {
		...
		Assert.Equal( "Demonstration terminal", first.Description );
		Assert.Equal( "representative.ti", first.Span.SourceName );
		...
		Assert.Equal( "xenl", second.Fields[ 0 ].CapabilityName );
	}
	...
	public void LexicalDiagnosticsAndMalformedHeadersRemainRecoverable() {
		...
		Assert.Equal( TermInfoSourceFieldKind.UseReference, inherited.Kind );
