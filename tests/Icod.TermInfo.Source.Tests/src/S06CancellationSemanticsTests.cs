...
public sealed class S06CancellationSemanticsTests {
	...
	public void LocalCancellationCreatesStandardAndExtendedTombstones() {
		...
		Assert.Contains( "VendorBool", state.CanceledExtendedCapabilities );
		Assert.Contains( "VendorNum", state.CanceledExtendedCapabilities );
		Assert.Contains( "VendorString", state.CanceledExtendedCapabilities );
	}
	...
	public void LaterLocalDeclarationReplacesEarlierValueOrCancellation() {
		...
		Assert.Equal( 132, state.NumericCapabilities[ NumericCapability.Columns ] );
		Assert.DoesNotContain(
			NumericCapability.Columns,
			state.CanceledNumericCapabilities
		);

		Assert.Equal( "done", state.StringCapabilities[ StringCapability.ClearScreen ] );
		Assert.DoesNotContain(
			StringCapability.ClearScreen,
			state.CanceledStringCapabilities
		);

		Assert.Equal( "final", state.ExtendedCapabilities[ "Vendor" ].StringValue );
		Assert.DoesNotContain( "Vendor", state.CanceledExtendedCapabilities );
	}
	...
	public void LocalCancellationsBlockInheritedValues() {
		...
		Assert.Contains( "Vendor", child.CanceledExtendedCapabilities );
	}
	...
	public void LocalValuesBlockInheritedCancellations() {
		...
		Assert.Equal( 132, child.NumericCapabilities[ NumericCapability.Columns ] );
		Assert.Equal( "child", child.StringCapabilities[ StringCapability.ClearScreen ] );
		Assert.Equal( "child", child.ExtendedCapabilities[ "Vendor" ].StringValue );
	...
	public void RightToLeftParentCompositionLetsLeftwardParentOverrideEarlierImport() {
		...
		Assert.Equal( 80, child.NumericCapabilities[ NumericCapability.Columns ] );
		Assert.Equal( "leftmost", child.StringCapabilities[ StringCapability.ClearScreen ] );
		Assert.Equal( "leftmost", child.ExtendedCapabilities[ "Vendor" ].StringValue );
	}
	...
	public void LeftwardParentCancellationOverridesRightmostSourceValue() {
		...
		Assert.DoesNotContain( "Vendor", parents.ExtendedCapabilities.Keys );
		Assert.Contains( "Vendor", parents.CanceledExtendedCapabilities );
	}
	...
	public void LeftwardParentValueOverridesRightmostSourceCancellation() {
		...
		Assert.Equal( 80, parents.NumericCapabilities[ NumericCapability.Columns ] );
		Assert.DoesNotContain(
			NumericCapability.Columns,
			parents.CanceledNumericCapabilities
		);
		Assert.Equal( "leftmost", parents.ExtendedCapabilities[ "Vendor" ].StringValue );
		Assert.DoesNotContain( "Vendor", parents.CanceledExtendedCapabilities );
	}
	...
	public void CloneCopiesValuesAndTombstonesWithoutSharingMutableState() {
		...
		Assert.Equal( "value", original.ExtendedCapabilities[ "Vendor" ].StringValue );
		Assert.Contains( "Other", original.CanceledExtendedCapabilities );
		Assert.DoesNotContain(
			StringCapability.ClearScreen,
			original.StringCapabilities.Keys
		);

		Assert.Equal( "lower", clone.StringCapabilities[ StringCapability.ClearScreen ] );
	}
