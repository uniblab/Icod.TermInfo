...
internal static class TermInfoParameterProgramAnalyzer {
	...
	private sealed class AnalysisEngine {
		...
		private bool ApplySimple(
			...
			switch ( instruction ) {
				...
				case TermInfoPushParameterInstruction pushParameter:
					RecordParameterReference( pushParameter.ParameterIndex );
					return Push(
						state,
						new AbstractValue(
		...
		private bool Push(
			...
			if ( state.Stack.Count >= TermInfoParameterLimits.MaximumStackDepth ) {
				throw new TermInfoFormatException(
		...
		private static List<AnalysisState> MergeStates(
			...
			Dictionary<(int StackDepth, int ImplicitIndex), AnalysisState> byShape =
				new();
	...
	private readonly record struct AbstractValue( int ParameterMask ) {
		...
		internal AbstractValue Merge( AbstractValue other ) {
			return new AbstractValue(
				ParameterMask | other.ParameterMask
			);
		}
	}
	...
	private sealed class AnalysisState {
		...
		internal AnalysisState Clone() {
			AnalysisState clone =
				new() {
					ImplicitParameterIndex = ImplicitParameterIndex,
				};
		...
		internal void MergeFrom( AnalysisState other ) {
			...
			if (
				...
				|| ImplicitParameterIndex != other.ImplicitParameterIndex
			) {
			...
			for ( int index = 0; index < Stack.Count; index++ ) {
				Stack[index] =
					Stack[index].Merge( other.Stack[index] );
			}

			for ( int index = 0; index < DynamicVariables.Length; index++ ) {
