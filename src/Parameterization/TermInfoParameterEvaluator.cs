...
internal sealed class TermInfoParameterEvaluator {
	...
	private void IncrementParameters(
		TermInfoIncrementParametersInstruction instruction
	) {
	...
	private void OutputCharacter(
		TermInfoCharacterOutputInstruction instruction
	) {
	...
	private void EvaluateConditional(
		TermInfoConditionalInstruction instruction
	) {
	...
	private void Push(
		...
		if ( _stack.Count >= TermInfoParameterLimits.MaximumStackDepth ) {
			throw new TermInfoEvaluationException(
	...
	private void AppendOutput(
		...
		if (
			value.Length
			> TermInfoParameterLimits.MaximumOutputLength - _output.Length
		) {
			throw new TermInfoEvaluationException(
	...
	private void AppendOutput(
		...
		if ( _output.Length >= TermInfoParameterLimits.MaximumOutputLength ) {
			throw new TermInfoEvaluationException(
				$"Expanded terminfo output cannot exceed "
				+ $"{TermInfoParameterLimits.MaximumOutputLength} characters",
				position
