...
internal sealed class TermInfoParameterParser {
	...
	internal IReadOnlyList<TermInfoInstruction> Parse() {
		ParseSequenceResult result =
			ParseSequence( TermInfoDelimiter.None );
	...
	private ParseSequenceResult ParseSequence(
		TermInfoDelimiter allowedDelimiters
	) {
	...
	private TermInfoInstruction ParseConditional( int percentPosition ) {
		...
		try {
			...
			while ( true ) {
				ParseSequenceResult elseCandidate =
					ParseSequence(
						TermInfoDelimiter.Then | TermInfoDelimiter.End
					);

				if (
					elseCandidate.Delimiter
					== TermInfoDelimiter.EndOfInput
				) {
					throw CreateFormatException(
						"Conditional expression is missing '%;'",
				...
				ParseSequenceResult elseIfBody =
					ParseSequence(
						TermInfoDelimiter.Else | TermInfoDelimiter.End
					);
				if (
					elseIfBody.Delimiter
					== TermInfoDelimiter.EndOfInput
				) {
					throw CreateFormatException(
						"Conditional expression is missing '%;'",
	...
	private TermInfoInstruction ParseVariable(
		...
		_position += 3;
		return ( set )
			? new TermInfoSetVariableInstruction(
				percentPosition,
				name
			)
			: new TermInfoGetVariableInstruction(
				percentPosition,
				name
			)
		;
	...
	private TermInfoInstruction ParseCharacterConstant( int percentPosition ) {
		...
		_position += 4;
		return new TermInfoPushCharacterInstruction(
			percentPosition,
			value
		);
	}
	...
	private TermInfoInstruction ParseIntegerConstant( int percentPosition ) {
		...
		_position = close + 1;
		return new TermInfoPushIntegerInstruction(
			percentPosition,
			value
		);
	}
	...
	private bool TryParseFormat(
		...
		if ( index > widthStart ) {
			...
			if ( parsedWidth > TermInfoParameterLimits.MaximumFormatWidth ) {
				throw CreateFormatException(
		...
		if ( index < _source.Length && _source[index] == '.' ) {
			...
			if ( parsedPrecision > TermInfoParameterLimits.MaximumFormatPrecision ) {
				throw CreateFormatException(
	...
	private static TermInfoFormatException CreateFormatException(
		...
		return new TermInfoFormatException(
			message,
			position
		);
	}
