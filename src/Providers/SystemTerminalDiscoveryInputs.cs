...
internal static class SystemTerminalDiscoveryInputs {
	...
	private static byte[] DecodeHex(
		...
		for ( int index = 0; index < decodedLength; index++ ) {
			int high =
				GetHexValue( payload[index * 2] );
			int low =
				GetHexValue( payload[( index * 2 ) + 1] );
	...
	private static byte[] DecodeBase64(
		...
		if ( firstPadding >= 0 ) {
			...
			explicitPadding =
				payload.Length - firstPadding;

			if ( explicitPadding > 2 || ( payload.Length & 3 ) != 0 ) {
				throw new FormatException(
					"The base64-encoded TERMINFO payload has invalid padding."
				);
			}

			for ( int index = firstPadding; index < payload.Length; index++ ) {
				if ( payload[index] != '=' ) {
					throw new FormatException(
						"The base64-encoded TERMINFO payload has invalid padding."
					);
		...
		for ( int index = 0; index < dataLength; index++ ) {
			if ( !IsBase64DataCharacter( payload[index] ) ) {
				throw new FormatException(
		...
		long normalizedLength =
			(long)payload.Length + implicitPadding;
		long decodedLength =
		...
		for ( int index = payload.Length; index < normalized.Length; index++ ) {
			normalized[index] = '=';
		}
	...
	private static bool IsBase64DataCharacter( char value ) {
